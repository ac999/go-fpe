...
func FF1Encrypt(K, T []byte, X string, radix, minlen, maxlen, maxTlen int) (string, error) {
	...
	radixBytes := intToNBytes(radix, 3)
	nBytes := intToNBytes(n, 4)
	tBytes := intToNBytes(t, 4)
	P := append([]byte{1, 2, 1}, radixBytes...)
	...
	for i := 0; i < 10; i++ {
		...
		if len(Q)%aes.BlockSize != 0 {
			return "", errors.New("invalid Q size: must be a multiple of 16 bytes")
		}
		...
		for j := 1; j < int(math.Ceil(float64(d)/float64(aes.BlockSize))); j++ {
			jBytes := intToNBytes(j, aes.BlockSize)
			for k, r := range R {
				R[k] = r ^ jBytes[k]
			}
			block.Encrypt(S, R)
