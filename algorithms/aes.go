package algorithms

// Implementation from FIPS 197
// AES block size is 16 bytes
const blockSize = 16

// S-box for SubBytes step
var sBox = [256]byte{
	,
}

// Rcon for the key expansion step
var rcon = [10]byte{0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36}

// Key expansion function
func keyExpansion(key []byte) [][]byte {
	numRounds := 10
	roundKeys := make([][]byte, numRounds+1)
	roundKeys[0] = make([]byte, len(key))
	copy(roundKeys[0], key)

	for i := 1; i <= numRounds; i++ {
		prev := roundKeys[i-1]
		temp := make([]byte, 4)

		// Rotate and substitute (the first word)
		temp[0], temp[1], temp[2], temp[3] = prev[13], prev[14], prev[15], prev[12]
		for j := 0; j < 4; j++ {
			temp[j] = sBox[temp[j]]
		}
		temp[0] ^= rcon[i-1]

		// Generate new round key
		roundKey := make([]byte, len(key))
		for j := 0; j < 4; j++ {
			roundKey[j] = prev[j] ^ temp[j]
		}
		for j := 4; j < len(key); j++ {
			roundKey[j] = prev[j] ^ roundKey[j-4]
		}
		roundKeys[i] = roundKey
	}

	return roundKeys
}

// AddRoundKey step
func addRoundKey(state, roundKey []byte) {
	for i := 0; i < blockSize; i++ {
		state[i] ^= roundKey[i]
	}
}

// SubBytes step
func subBytes(state []byte) {
	for i := 0; i < blockSize; i++ {
		state[i] = sBox[state[i]]
	}
}

// ShiftRows step
func shiftRows(state []byte) {
	temp := make([]byte, blockSize)
	copy(temp, state)

	// Perform shifts
	state[1], state[5], state[9], state[13] = temp[5], temp[9], temp[13], temp[1]
	state[2], state[6], state[10], state[14] = temp[10], temp[14], temp[2], temp[6]
	state[3], state[7], state[11], state[15] = temp[15], temp[3], temp[7], temp[11]
}

// MixColumns step
func mixColumns(state []byte) {
	mul := func(a, b byte) byte {
		var p byte
		for b > 0 {
			if b&1 == 1 {
				p ^= a
			}
			if a&0x80 != 0 {
				a = (a << 1) ^ 0x1B
			} else {
				a <<= 1
			}
			b >>= 1
		}
		return p
	}

	for i := 0; i < 4; i++ {
		col := state[i*4 : (i+1)*4]
		temp := make([]byte, 4)
		copy(temp, col)

		col[0] = mul(temp[0], 2) ^ mul(temp[1], 3) ^ temp[2] ^ temp[3]
		col[1] = temp[0] ^ mul(temp[1], 2) ^ mul(temp[2], 3) ^ temp[3]
		col[2] = temp[0] ^ temp[1] ^ mul(temp[2], 2) ^ mul(temp[3], 3)
		col[3] = mul(temp[0], 3) ^ temp[1] ^ temp[2] ^ mul(temp[3], 2)
	}
}

// AES encryption
func AesEncrypt(input, key []byte) []byte {
	state := make([]byte, blockSize)
	copy(state, input)

	roundKeys := keyExpansion(key)

	// Initial round
	addRoundKey(state, roundKeys[0])

	// Main rounds
	for round := 1; round <= 9; round++ {
		subBytes(state)
		shiftRows(state)
		mixColumns(state)
		addRoundKey(state, roundKeys[round])
	}

	// Final round
	subBytes(state)
	shiftRows(state)
	addRoundKey(state, roundKeys[10])

	return state
}
